...
local BaseSymbolUnit = import(".BaseSymbolUnit")
local BaseReelUnit = class("BaseReelUnit",cc.Node)


-- 运动状态 --
BaseReelUnit.RollStatus = 
{
...
function BaseReelUnit:ctor( reelConfig,reelIndex )
...
-- ################################ 初始化相关 ################################
-- 检查参数是否正确
function BaseReelUnit:checkParam( param )
...
function BaseReelUnit:initReelParam()
...
-- 初始化裁剪区域
function BaseReelUnit:addClipNode()
	-- 添加裁剪node
...
-- 添加信号块
function BaseReelUnit:initSymbol()
	-- 不存在
...
function BaseReelUnit:createSymbolUnit( symbolId,posY )
	assert( symbolId," !! symbolId is nil !! ")
	assert( posY," !! posY is nil !! " )
	local symbol = self:createSingleSymbol()
...
-- 子类必须重写
function BaseReelUnit:createSingleSymbol()
	local symbol = BaseSymbolUnit.new( self._symbolSize,self._reelConfig )
	return symbol
end
...
function BaseReelUnit:getPosYFromSecond( index )
...
-- 根据信号源id获得当前Symbol尺寸
function BaseReelUnit:getSymbolHeightById( symbolId )
	assert( symbolId," !! symbolId is nil !! " )
	return self._reelConfig.size_type[symbolId] * self._symbolSize.height
end
-- 根据信号源id获得当前Symbol的size_type
function BaseReelUnit:getSymbolSizeTypeById( symbolId )
	assert( symbolId," !! symbolId is nil !! " )
...
function BaseReelUnit:updataSymbolUnit( dt )
	-- 设置位置
...
function BaseReelUnit:getTopPosY()
	local topY = 0
	local symbolId = 0
	for i,v in ipairs( self._symbolList ) do
...
-- ################################ 外部接口调用 ################################
-- 开启滚动
function BaseReelUnit:startRoll()
...
-- 停止滚动
function BaseReelUnit:stopRoll( destData )
	-- 需要变换数据 得到需要更改的索引位置
...
-- 起跳动作
function BaseReelUnit:startJumpAction()
	for i,v in ipairs(self._symbolList) do
...
-- 停止之后 校正位置
function BaseReelUnit:reviseSymbolPosition( diff )
	if diff <= 0 then
		return
	end
	for i,v in ipairs(self._symbolList) do
