...
local NodePoker = import("app/viewsminigame/viewszhandou/NodePoker")
...
function GamePlay:gameOver( winType )
	...
	if winType == 1 then
		-- ai赢
		addUIToScene( UIDefine.MINIGAME_KEY.ZhanDou_Lose_UI )
	elseif winType == 2 then
		-- 玩家赢
		G_GetModel("Model_ZhanDou"):setStage()
		addUIToScene( UIDefine.MINIGAME_KEY.ZhanDou_Win_UI,{ score = G_GetModel("Model_ZhanDou"):getScore() } )
...
function GamePlay:clickPause()
	self:stopSchedule()
	addUIToScene( UIDefine.MINIGAME_KEY.ZhanDou_Pause_UI,{ layer = self } )
end

function GamePlay:close()
	removeUIFromScene( UIDefine.MINIGAME_KEY.ZhanDou_Play_UI )
    addUIToScene( UIDefine.MINIGAME_KEY.ZhanDou_Start_UI )
end
