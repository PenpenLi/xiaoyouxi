...
function GamePlay:ctor( param )
    ...
    self:addNodeClick( self["ButtonHelp"],{ 
        endCallBack = function() addUIToScene( UIDefine.MINIGAME_KEY.JunShi_Help_UI ) end
    })
end
...
function GamePlay:junshiChangeAction( poker,num )
	...
	local call = cc.CallFunc:create( function()
		if self._pointer == 4 then
			-- 玩家需要打开选择界面
			addUIToScene( UIDefine.MINIGAME_KEY.JunShi_Choose_UI,{ 
...
function GamePlay:gameOver()
	...
	addUIToScene( UIDefine.MINIGAME_KEY.JunShi_Over_UI,data )
end

function GamePlay:close()
	removeUIFromScene( UIDefine.MINIGAME_KEY.JunShi_Play_UI )
    addUIToScene( UIDefine.MINIGAME_KEY.JunShi_Start_UI )
end
