...
function GamePlay:sendCardToPlayer( seatPos,callBack,isPlayerVoice )
	...
	if self:isGameDisband() then
		-- 弹出流局界面
		addUIToScene( UIDefine.MINIGAME_KEY.Eight_Disband_UI )
		return
	end
...
function GamePlay:playerOutCard( poker )
	...
	if poker:getCardNum() == 8 then
		addUIToScene( UIDefine.MINIGAME_KEY.Eight_Choice_UI,{ poker = poker,parent = self,seatPos = 4 } )
		return
	end
...
function GamePlay:gameOver( seatPos )
	...
	addUIToScene( UIDefine.MINIGAME_KEY.Eight_Over_UI,{result = playerAllNumIndex} )
end
...
function GamePlay:pauseGame()
	self:pause()--暂停
	addUIToScene( UIDefine.MINIGAME_KEY.Eight_Stop_UI,self )
end
