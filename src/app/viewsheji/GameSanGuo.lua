...
local NodePoker = import(".NodeSanGuoPoker")

local GameSanGuo = class("GameSanGuo",BaseLayer)


function GameSanGuo:ctor( param )
    assert( param," !! param is nil !! ")
    assert( param.name," !! param.name is nil !! ")
    GameSanGuo.super.ctor( self,param.name )

    self:addCsb( "csbheji/csbsanguo/Play.csb" )
...
function GameSanGuo:onEnter()
	GameSanGuo.super.onEnter( self )
...
function GameSanGuo:sendCardAction()
...
function GameSanGuo:sendOneCardAction( seatPos )
	...
	if G_GetModel("Model_Sound"):isVoiceOpen() then
		audio.playSound("csbheji/csbsanguo/sgmp3/sendcard.mp3", false)
	end
...
-- 将ai手中的牌 平铺
function GameSanGuo:aiHandPokerAction( seatPos )
	assert( seatPos," !! seatPos is nl !! " )
	local dest_node = self["NodeAI"..seatPos]
...
function GameSanGuo:getCardPosBySeat( seatPos )
...
-- AI出牌
function GameSanGuo:aiOutPoker()
	local dest_node = self["NodeAI"..self._pointer]
	-- AI逻辑
...
-- 玩家出牌
function GameSanGuo:playerOutPoker()
	-- 1:将不能出的牌置灰
	local cur_data = sanguo_config.card[self._curOutPokerIndex]
...
function GameSanGuo:aiOrPlayerSelectOutPoker( poker )
			...
			elseif out_data.index == 13 then
				...
				local call_set = cc.CallFunc:create( function()
					if self._pointer ~= 4 then
						...
						-- 玩家 选择颜色
						addUIToScene( UIDefine.HEJI_KEY.SanGuo_Select_UI,{ callBack = function( colorNumIndex )
							-- 重置
							if self._pointer == 4 then
								self._moveAction = nil
							end
...
-- 检查玩家选择的牌能否出
function GameSanGuo:checkSelectPokerCanOut( poker )
	assert( poker," !! poker is nil !! " )
	if self._pointer ~= 4 then
		return false
	end
...
-- 检查玩家能否出牌
function GameSanGuo:checkAIPlayerCanOut( seatPos )
	assert( seatPos," !! seatPos is nil !! " )
	local dest_node = self["NodeAI"..seatPos]
	local cur_data = sanguo_config.card[self._curOutPokerIndex]
	local childs = dest_node:getChildren()
...
-- 改变指向
function GameSanGuo:changePointer()
	if self._dir == 1 then
		if self._pointer == 4 then
			self._pointer = 1
		else
...
function GameSanGuo:showDirAndPointer()
	-- 重置
	for i = 1,4 do
		self["ImageDir"..i]:stopAllActions()
		self["ImageDir"..i]:setVisible( true )
		self["ImageDir"..i]:setRotation( 0 )
		self["ImageDir"..i]:loadTexture( "imagesanguo/play/ni1.png",1 )
	end
	if self._dir == 1 then
		...
		self["ImageDir"..self._pointer]:loadTexture( "imagesanguo/play/ni2.png",1 )
	else
		...
		self["ImageDir"..dir_index]:loadTexture( "imagesanguo/play/ni2.png",1 )
	end
end

function GameSanGuo:createTurnLayer( callBack )
	local layer = cc.LayerColor:create( cc.c4b( 0,0,0,150 ) )
	self:addChild( layer,10 )
	local img = ccui.ImageView:create( "imagesanguo/play/change.png",1 )
...
function GameSanGuo:createPlayerGetCard( callBack )
	local layer = cc.LayerColor:create( cc.c4b( 0,0,0,150 ) )
	self:addChild( layer,10 )
	local img = ccui.ImageView:create( "imagesanguo/play/yun1.png",1 )
	self:addChild( img,20 )
	img:setPosition( display.cx,display.cy )
	local text = ccui.Text:create()
	text:setString("需要摸一张牌")
	text:setFontName("imagesanguo/sanguo.ttf")
	text:setFontSize( 40 )
...
function GameSanGuo:gameOver( resultType )
	if resultType == 1 then
		-- ai 赢
		G_GetModel("Model_Heji"):setCoin( -10 )
		addUIToScene( UIDefine.HEJI_KEY.SanGuo_Lose_UI )
	elseif resultType == 2 then
		-- 玩家赢
		G_GetModel("Model_Heji"):setCoin( 10 )
		addUIToScene( UIDefine.HEJI_KEY.SanGuo_Win_UI )
	elseif resultType == 3 then
		-- 流局
		addUIToScene( UIDefine.HEJI_KEY.SanGuo_LiuJu_UI )
	end
end

function GameSanGuo:setPlayerSecect( poker )
	assert( poker," !! poker is nil !! ")
	self._playerSelectPoker = poker
end

function GameSanGuo:getPlayerSecect()
	return self._playerSelectPoker
end

function GameSanGuo:clearPlayerSecect()
