...
local GameTwentyOne = class("GameTwentyOne",BaseLayer)


function GameTwentyOne:ctor( param )
    assert( param," !! param is nil !! ")
    assert( param.name," !! param.name is nil !! ")
    GameTwentyOne.super.ctor( self,param.name )
    self:addCsb( "csbheji/csbtwentyone/Play.csb" )
    ...
    self:addNodeClick( self.ButtonHelp,{
    	endCallBack = function() addUIToScene( UIDefine.HEJI_KEY.TwentyOne_Help_UI ) end
    })
...
function GameTwentyOne:addHandAction()
...
function GameTwentyOne:removeHandAction()
...
function GameTwentyOne:loadaImagePoker()
...
function GameTwentyOne:touchPanel( index )
...
function GameTwentyOne:calResult( index )
...
function GameTwentyOne:calPoint( index )
	assert( index," !! index is nil !! " )
	local result = {}
	local has_a = false
...
function GameTwentyOne:addBeiPoker()
	self.ImagePokerBei:setVisible( false )
	local pos = cc.p( self.ImagePokerBei:getPosition() )
	for i = 1,51 do
		local bei_poker = ccui.ImageView:create( "imagetwntyone/poker/bei.png",1 )
...
function GameTwentyOne:addAiXin( delayTime )
	self.ImageTiZi:stopAllActions()
	if self._aixinPs == nil then
		self._aixinPs = cc.ParticleSystemQuad:create("imagetwntyone/ainxin.plist")
...
function GameTwentyOne:gongZhuMoveAction()
...
function GameTwentyOne:gongZhuMoveRightAction()
	...
	for i = 1,4 do
		...
		local change_icon = cc.CallFunc:create( function()
			self.ImgGongZhu:loadTexture( "imagetwntyone/game/gongzhu"..i..".png",1 )
		end )
...
function GameTwentyOne:gongZhuMoveLeftAction()
	self.ImgGongZhu:stopAllActions()
	local actions = {}
	local time = 0.1
	for i = 5,7 do
...
function GameTwentyOne:solderMoveAction( isMoveTwo )
	local rate = 1
	if isMoveTwo then
		rate = 2
	end
...
function GameTwentyOne:solderFrameAction()
	self.ImageSolder:stopAllActions()
	local actions = {}
	local time = 0.1
	for i = 1,3 do
		local delay = cc.DelayTime:create(time)
		local change_icon = cc.CallFunc:create( function()
			self.ImageSolder:loadTexture( "imagetwntyone/game/qs"..i..".png",1 )
		end )
		table.insert( actions,delay )
		table.insert( actions,change_icon )
	end
...
function GameTwentyOne:addExtreJinDuAction()
...
-- 刷新当前的扑克
function GameTwentyOne:refreshCurrentPoker( removeBei )
...
function GameTwentyOne:showGameOver()
	-- 检查是否结束游戏
	if self._curTiZi >= self._totalTiZi then
		...
		self.ImgGongZhu:loadTexture("imagetwntyone/game/qishigongzhu.png",1)
		self.NodeGongZhu:setPositionX( 120 )
		self:addAiXin()

		performWithDelay( self._csbNode,function()
			local data = { score = self._score }
			addUIToScene( UIDefine.HEJI_KEY.TwentyOne_Pass_UI,data )
		end,2 )
	else
		if self._pokerNone then
			-- 游戏结束
			local data = { score = self._score }
			addUIToScene( UIDefine.HEJI_KEY.TwentyOne_Over_UI,data )
			self.ImgJinDuBg:setVisible( false )
			self.LoadingJinDu:stopAllActions()
			self.ImgGongZhu:stopAllActions()
			self.NodeGongZhu:stopAllActions()
...
function GameTwentyOne:addListener()
