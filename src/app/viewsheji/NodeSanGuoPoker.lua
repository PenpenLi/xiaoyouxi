...
local NodeSanGuoPoker = class( "NodeSanGuoPoker",BaseNode )




function NodeSanGuoPoker:ctor( parentPanel,beiIndex )
	self._parentPanel = parentPanel
	NodeSanGuoPoker.super.ctor( self,"NodeSanGuoPoker" )
...
function NodeSanGuoPoker:loadDataUI( numIndex )
	assert( numIndex," !! numIndex is nil !! " )
	self._numIndex = numIndex
end

function NodeSanGuoPoker:showPoker()
...
function NodeSanGuoPoker:removePokerClick()
	self._image:setTouchEnabled( false )
end

function NodeSanGuoPoker:playerOut()
...
function NodeSanGuoPoker:playerCancel()
	local select_poker = self._parentPanel:getPlayerSecect()
	if select_poker then
		local move_by = cc.MoveBy:create( 0.2,cc.p( 0,-20 ) )
		select_poker:runAction( move_by )
...
function NodeSanGuoPoker:getNum()
